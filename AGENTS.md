...
- Access is invite-only. First-run `/setup` (`bootstrapHousehold`) creates the admin
  ...
  aborts in-flight provider HTTP via AbortSignal, discards the preview,
  and does not replace prior IR; cancelled is a calm status, not an error
  toast; delete/rename wait on the generate lock so a late commit cannot
  resurrect an id; subject
