...
describe('examify-ingest generate', () => {
  ...
  it('does not create a missing parent when mkdir is false', () => {
    const root = examifyRepo();
    const dest = path.join(root, 'content/subjects/gone/bank.ir.json');
    expect(() => writeFileAtomic(dest, '{ "ok": true }\n', { mkdir: false })).toThrow();
    expect(existsSync(path.join(root, 'content/subjects/gone'))).toBe(false);
  });
